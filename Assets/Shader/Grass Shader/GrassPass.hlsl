////Credits to Youtube.com/AJTECH -> https://www.youtube.com/watch?v=NMlVZYvspJk
...
void LitPassGeom(triangle Varyings input[3], inout TriangleStream<Varyings> outStream)
{
    ...
    if(_Wind >=1)
    {
        windSample = tex2Dlod(_WindDistortionMap, float4(uv,0,0)) * _WindStrength;
    }
    else
    {
        windSample = float4(0,0,0,0);
    }

    float3 rotatedNormalZ = mul(input[0].normalWS, RotZ(windSample.x));
    float3 rotatedNormal = mul(rotatedNormalZ, RotX(windSample.y));

    float randomHeight = rand(input[0].positionWSAndFogFactor.xyz * 3.214124f);
